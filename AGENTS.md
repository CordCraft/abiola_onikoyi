...
- **Auth**: everyday sign-in at `/mentorship/login` is email + password (bcrypt `passwordHash`). First-timers self-register at `/mentorship/join`: a 4-step wizard (account, photo + contact, background story + direction, programme expectations) that creates or claims the mentee row by email (claims only rows without a password; admin pre-registration is optional), sets `onboardedAt`, then `draftInitialGoals()` (`src/lib/mentorship/goal-draft.ts`) generates 3 starter goals from the story via `claude-opus-4-8` (plain-JSON pattern like generate-post.ts; template fallback without `ANTHROPIC_API_KEY` or on error; never throws). Recovery: admin "Reset password" clears `passwordHash` and the mentee registers again (profile preserved). Access codes still exist on rows but are no longer used to log in. Photos are browser-compressed to ~100KB jpeg data URLs stored in `photoData` (no blob store). Separate jose cookie `mentee_session` (payload `{menteeId, name, expiresAt}`, signed with the same `SESSION_SECRET`; `decryptMenteeSession` rejects admin tokens and vice versa). Optimistic redirects live in `src/proxy.ts`; the authoritative check is `verifyMentee()` in `src/lib/mentorship/dal.ts`, which redirects stale sessions through the `/mentorship/logout` route handler (cookies cannot be mutated during page render).
