    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6">
        ...
        <p className="text-sm text-zinc-500">
          Files captured through chat live here. Jarvis can search and re-read them any time.
        </p>
        <ul className="mt-4 space-y-3">
          {project.documents.length === 0 ? (
            <li className="text-sm text-zinc-500">
              No documents yet. Attach files in the chat and Jarvis will file them here.
            </li>
          ) : null}
          {project.documents.map((doc) => (
            <li key={doc.id} className="flex items-start justify-between gap-3 rounded-lg border border-zinc-100 px-3 py-2.5">
              <div className="min-w-0">
                <Link href={`/jarvis/documents/${doc.id}`} className="flex items-center gap-1.5 text-sm font-medium text-zinc-900 hover:text-indigo-700">
                ...
                {doc.summary ? <p className="mt-1 text-sm text-zinc-600">{doc.summary}</p> : null}
                <p className="mt-0.5 text-xs text-zinc-400">{formatDate(doc.createdAt)}</p>
              </div>
              <form action={deleteDocument}>
                <input type="hidden" name="id" value={doc.id} />
                <ConfirmSubmit message="Delete this document?" className="text-xs text-zinc-400 hover:text-red-600">
                  ×
                </ConfirmSubmit>
              </form>
            </li>
